...
<form action="<?=$_SERVER['PHP_SELF']?>" method="post">
  ...
  <input type="submit" class="ui-button ui-corner-all" value="<?=_("Yes, delete the tag")?>">
</form>
...
<form action="<?=$_SERVER['PHP_SELF']?>" method="post">
  ...
  <input type="hidden" name="confirmed" value="1">
  <input type="submit" class="ui-button ui-corner-all" value="<?=_("Yes, delete the event and history records")?>">
</form>
<?php
      $need_confirmation = 1;
    }
...
// ********** USER ADD/UPDATE (admin only) **********
  ...
  if ($_SESSION['access'] != 2) {
    ...
    $adminlevel = intval($_POST['accesslevel']);
    if ($_POST['userid'] == "new") {
      ...
      if (mysqli_num_rows($result) > 0) {
        ...
        sqlquery_checked("INSERT INTO user (UserID,UserName,Password,Access,Language) ".
      ...
      if (empty($message)) {
        ...
        $sql .= "Access=$adminlevel,Language='".h2d($_POST['language'])."' WHERE UserID='$old_userid'";
