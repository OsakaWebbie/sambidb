...
if ($sid) {
  $sql = "SELECT * FROM pw_song WHERE SongID=$sid";
...
<table border=0 cellspacing=0 cellpadding=5><tr><td nowrap>
  Title: <input name=title type=text size=50 maxlength=50 value="<? echo $rec->Title; ?>"
  onchange="editform.updated.value=1;" style="ime-mode:auto;">
</td><td nowrap>
  Original Title: <input name=origtitle type=text size=40 maxlength=50 value="<? echo $rec->OrigTitle; ?>"
...
Composer: <input name=composer type=text size=45 maxlength=80 value="<? echo $rec->Composer; ?>"
    onchange="editform.updated.value=1;" style="ime-mode:auto;"><br>
Copyright: <input name=copyright type=text size=45 maxlength=80 value="<? echo $rec->Copyright; ?>"
    onchange="editform.updated.value=1;" style="ime-mode:auto;"><br>
Key: <input name=songkey type=text size=10 maxlength=8 value="<? echo $rec->SongKey; ?>"
...
<option></option>
<option value="Fast"<? if ($rec->Tempo=="Fast") echo " selected"; ?> >Fast</option>
<option value="Medium"<? if ($rec->Tempo=="Medium") echo " selected"; ?> >Medium</option>
<option value="Slow"<? if ($rec->Tempo=="Slow") echo " selected"; ?> >Slow</option>
</select>
</td><td>
Source:<br><textarea rows=4 name=source cols=50
onchange="editform.updated.value=1;"><? echo $rec->Source; ?></textarea></td>
</tr></table><br />
<div align="center"><table border=0 cellspacing=0 cellpadding=5><tr><td valign="top"> 
    ...
    Pattern for printing (e.g. ABABB; sections are divided by blank lines):<br><input name=pattern type=text size=30 maxlength=80 value="<? echo $rec->Pattern; ?>"
    ...
    onchange="editform.updated.value=1;"><? echo $rec->Instruction; ?></textarea><br />&nbsp;<br />
    Upload audio file (MP3 format only, <span style="color:darkred;font-weight:bold">no larger than <? echo ini_get("upload_max_filesize"); ?> file size or you will lose other data you have edited!</span>):<br>
    <input name=audiofile type=file size=35 onchange="editform.updated.value=1;"><br />
    <input type=hidden name=audio value="<? echo $rec->Audio; ?>">
<? if ($rec->Audio == "1") {
...
?>
    ...
    onchange="editform.updated.value=1;"><? echo $rec->AudioComment; ?></textarea>
  </td><td>
    ...
    onchange="editform.updated.value=1;"><? echo $rec->Lyrics; ?></textarea></td></tr></table></div>
</form>
