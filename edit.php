...
function validate() {
  ...
  if (f.updated.value == 0) {
    alert("<?php echo _('No info was modified.  If you want to exit this page, just use your BACK button.'); ?>");
    return false;
  }
  if (f.title.value.length == 0) {
    alert("<?php echo _('Please enter the title!'); ?>");
    f.title.select();
    return false;
  }
  if ((f.audiofile.value) && (!mp3_regexp.test(f.audiofile.value))) {
    alert("<?php echo _('Only MP3 files can be accepted for audio.'); ?>");
    f.audiofile.value = "";
    return false;
  }
<?php
...
<table border=0 cellspacing=0 cellpadding=5><tr><td nowrap>
  ...
  <?php echo _('Original Title'); ?>: <input name=origtitle type=text size=40 maxlength=50 value="<?php echo $rec->OrigTitle; ?>"
  onchange="editform.updated.value=1;" style="ime-mode:auto;"><br>
  <div align=left><font color=red size=-2><?php echo _('(Fill in if song is translated)'); ?></div>
</td></tr></table>
<table border=0 cellspacing=0 cellpadding=5><tr><td>
<?php echo _('Composer'); ?>: <input name=composer type=text size=45 maxlength=80 value="<?php echo $rec->Composer; ?>"
    onchange="editform.updated.value=1;" style="ime-mode:auto;"><br>
<?php echo _('Copyright'); ?>: <input name=copyright type=text size=45 maxlength=80 value="<?php echo $rec->Copyright; ?>"
    onchange="editform.updated.value=1;" style="ime-mode:auto;"><br>
<?php echo _('Key'); ?>: <input name=songkey type=text size=10 maxlength=8 value="<?php echo $rec->SongKey; ?>"
    onchange="editform.updated.value=1;" style="ime-mode:disabled;">&nbsp;&nbsp;&nbsp;&nbsp;
<?php echo _('Tempo'); ?>: <select name=tempo size=1 onchange="editform.updated.value=1;">
<option></option>
<option value="Fast"<?php if ($rec->Tempo=="Fast") echo " selected"; ?> ><?php echo _('Fast'); ?></option>
<option value="Medium"<?php if ($rec->Tempo=="Medium") echo " selected"; ?> ><?php echo _('Medium'); ?></option>
<option value="Slow"<?php if ($rec->Tempo=="Slow") echo " selected"; ?> ><?php echo _('Slow'); ?></option>
</select>
</td><td>
<?php echo _('Source'); ?>:<br><textarea rows=4 name=source cols=50
...
<div align="center"><table border=0 cellspacing=0 cellpadding=5><tr><td valign="top">
    <input type=submit value="<?php echo _('  Save Changes  '); ?>" name=edit><br />&nbsp;<br />
    <?php echo _('Pattern for printing (e.g. ABABB; sections are divided by blank lines)'); ?>:<br><input name=pattern type=text size=30 maxlength=80 value="<?php echo $rec->Pattern; ?>"
...
<?php if ($rec->Audio == "1") {
  echo "<font size=\"-2\">"._('(This song already has audio uploaded, but you can replace it with a new file if you want.)')."</font><br />\n";
}
?>
    <?php echo _('Audio Comment'); ?>: <textarea rows=2 name=audiocomment cols=30
    onchange="editform.updated.value=1;"><?php echo $rec->AudioComment; ?></textarea>
  </td><td>
    <table border=0 cellspacing=0 cellpadding=0><tr><td valign=middle><?php echo _('Lyrics'); ?>:&nbsp;&nbsp;</td>
    <td style="color:red;textsize:0.8em"><ul><li><?php echo _('To add chords, enclose them in [brackets] just before the syllable they are played with.'); ?></li><li><?php echo _('If there are parts that are only needed to print full pattern (like a chorus with a tag), precede them with a line with only hyphens (one or more - it doesn\'t matter how many), like "---".'); ?></li></ul></td>
    </tr></table>
